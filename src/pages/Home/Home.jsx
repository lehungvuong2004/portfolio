...
import { usePageAnimation, useHeroTimeline, useCountUp, useMagneticEffect } from '../../gsap';
...
const PortfolioHome = () => {
  ...
  // Granular refs for orchestrated hero timeline
  ...
  // Premium orchestrated hero timeline
  useHeroTimeline({
    badge: badgeRef,
    title: titleRef,
  ...
  // Animated counters
  const countMonths = useCountUp(3, { duration: 1.5 });
  const countProjects = useCountUp(10, { duration: 2 });

  // Magnetic CTA button
  const magneticBtn = useMagneticEffect(0.35);

  const heroSection = () => (
    <section className="relative pt-32 pb-16 lg:pt-48 lg:pb-32 overflow-hidden">
      {/* Background Effects — pointer-events-none, transform-only */}
      <div className="absolute top-1/4 left-0 w-[31.25rem] h-[31.25rem] bg-blue-600/20 rounded-full blur-[7.5rem] -z-10 mix-blend-screen pointer-events-none will-change-transform"></div>
      <div className="absolute bottom-0 right-0 w-[37.5rem] h-[37.5rem] bg-cyan-600/10 rounded-full blur-[9.375rem] -z-10 mix-blend-screen pointer-events-none will-change-transform"></div>
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-16 flex flex-col lg:grid lg:grid-cols-2 gap-16 lg:gap-8 items-center mt-2 lg:mt-0">
        ...
        <div className="order-1 flex flex-col">
          ...
          {/* Title */}
          <h1 ref={titleRef} className="text-5xl md:text-7xl font-black leading-[1.1] mb-6 text-gray-900 dark:text-white">
            {t('home.hello')} <br />
          ...
          {/* Subtitle */}
          <h2 ref={subtitleRef} className="text-2xl md:text-3xl font-bold text-gray-600 dark:text-gray-400 mb-8">
            {t('home.role')}
          </h2>

          {/* Desc */}
          <p ref={descRef} className="text-gray-600 dark:text-gray-400 text-lg leading-relaxed max-w-xl mb-12 border-l-2 border-blue-500/50 dark:border-cyan-500/50 pl-6">
            {t('home.desc')}
          </p>

          {/* CTA */}
          <div ref={ctaRef} className="flex flex-wrap gap-5 mb-16">
            <div ref={magneticBtn}>
              <Link
                to="/contacts"
                className="group relative inline-flex items-center justify-center gap-2 bg-gray-900 dark:bg-white text-white dark:text-black px-8 py-4 rounded-xl font-black overflow-hidden transition-all hover:scale-105 active:scale-95"
              >
                ...
                <MyIcon
                  ...
                  className="relative group-hover:text-white transition-colors duration-300 group-hover:translate-x-1"
                  size={20}
                />
              </Link>
            </div>
        ...
        {/* IMAGE */}
        <div ref={imageRef} className="relative flex justify-center lg:justify-end order-2 lg:row-span-2">
          <div className="relative w-full max-w-md">
            {/* Decorative glow */}
            <div className="absolute inset-0 bg-linear-to-tr from-cyan-500 to-purple-600 rounded-[2.5rem] blur-2xl opacity-20 transform rotate-3 scale-105 pointer-events-none"></div>
            ...
            <div className="absolute -bottom-8 -left-8 md:-left-12 bg-white/90 dark:bg-[#11161d]/90 backdrop-blur-xl p-5 rounded-2xl border border-gray-200 dark:border-white/10 flex items-center gap-5 shadow-[0_10px_30px_rgba(0,0,0,0.1)] dark:shadow-[0_20px_40px_rgba(0,0,0,0.4)] animate-bounce-slow">
              ...
              <div>
                <div className="text-[0.625rem] text-gray-500 dark:text-gray-400 uppercase font-black tracking-[0.2em] mb-1">{t('home.coreSkills')}</div>
        ...
        {/* STATS */}
        <div ref={statsRef} className="order-3 flex gap-12 lg:gap-16 border-t lg:border-t-0 border-gray-200 dark:border-white/10 pt-10 lg:pt-0">
          <div className="stat-item">
            <div className="text-5xl font-black text-gray-900 dark:text-white flex items-center">
              <span ref={countMonths}>3</span>
              <span className="text-blue-500 dark:text-cyan-400">+</span>
            </div>
            <div className="text-gray-500 text-[0.625rem] uppercase tracking-[0.2em] mt-3 font-bold">{t('home.monthsExp')}</div>
          </div>
          <div className="stat-item">
            <div className="text-5xl font-black text-gray-900 dark:text-white flex items-center">
              <span ref={countProjects}>10</span>
              <span className="text-purple-500 dark:text-purple-400">+</span>
            </div>
            <div className="text-gray-500 text-[0.625rem] uppercase tracking-[0.2em] mt-3 font-bold">{t('home.projectsCompleted')}</div>
          </div>
        </div>
